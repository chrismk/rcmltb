...
async def stats(client, message):
    ...
    stats = f'<b>程序编译时间:</b> {last_commit}\n\n'\
            f'<b>机器人已运行:</b> {get_readable_time(time() - botUptime)}\n'\
            f'<b>服务器已运行:</b> {get_readable_time(time() - boot_time())}\n\n'\
            ...
            f'<b>已用:</b> {get_readable_file_size(memory.used)} | <b>剩余:</b> {get_readable_file_size(memory.available)}\n\n'\
